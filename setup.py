...
third_party_libs = ["kenlm", "openfst-1.6.9/src/include", "ThreadPool", "boost_1_69_0", "utf8"]
...
lib_sources = glob.glob('third_party/kenlm/util/*.cc') + glob.glob(
    'third_party/kenlm/lm/*.cc') + glob.glob('third_party/kenlm/util/double-conversion/*.cc'
                                             ) + glob.glob('third_party/openfst-1.6.9/src/lib/*.cc')
...
setup(
    ...
    ext_modules=[
        CppExtension(
            name='ctcdecode._ext',
            sources=glob.glob('ctcdecode/src/*.cpp') + lib_sources,
